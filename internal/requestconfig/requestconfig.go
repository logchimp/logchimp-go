...
func getDefaultHeaders() map[string]string {
	return map[string]string{
		"User-Agent": fmt.Sprintf("Logchimp/Go %s", internal.PackageVersion),
	}
}
...
func SubstituteServerVariables(templateURL *url.URL, cfg *RequestConfig) (*url.URL, error) {
	...
	if strings.Count(baseURL, "PROTOCOL") >= 1 {
		if cfg.Protocol == "" {
			return nil, fmt.Errorf("must provide Protocol to substitute %s", baseURL)
		}
		baseURL = strings.ReplaceAll(baseURL, "PROTOCOL", cfg.Protocol)
	}
	if strings.Count(baseURL, "HOST_WITH_PORT") >= 1 {
		if cfg.HostWithPort == "" {
			return nil, fmt.Errorf("must provide HostWithPort to substitute %s", baseURL)
		}
		baseURL = strings.ReplaceAll(baseURL, "HOST_WITH_PORT", cfg.HostWithPort)
	}
